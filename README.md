...
已知问题：有些机器，宝塔安装的Imagick拓展居然不支持WEBP格式，导致图床无法正常使用，可以参考一下这个篇文章升级Imagick版本（https://dev.euyyue.com/note/379.html），但是升级之后还是不支持webp，那我就不知道怎么办了。我在other文件夹下放了一个test.php文件，浏览器访问它能看到你安装的拓展是否支持WEBP。
