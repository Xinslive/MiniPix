...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['image'])) {
        ...
        $datePath = date('Y/m/d');
        $newFilePathWithoutExt = $uploadDir . $randomFileName;
        ...
        if (move_uploaded_file($file['tmp_name'], $newFilePath)) {
            ...
            if ($fileMimeType === 'image/png') {
                $convertSuccess = convertPngWithImagick($newFilePath, $uploadDir . $randomFileName . '.webp', $quality);
                if ($convertSuccess) {
                    $finalFilePath = $uploadDir . $randomFileName . '.webp';
                }
            } elseif ($fileMimeType !== 'image/webp' && $fileMimeType !== 'image/gif') {
                $convertSuccess = convertToWebp($newFilePath, $uploadDir . $randomFileName . '.webp', $quality);
                if ($convertSuccess) {
                    $finalFilePath = $uploadDir . $randomFileName . '.webp';
                }
            }
            ...
            try {
                ...
                $ossFilePath = 'img/' . $datePath . '/' . basename($finalFilePath);
                ...
                $stmt = $mysqli->prepare("INSERT INTO images (url, path) VALUES (?, ?)");
                $stmt->bind_param("ss", $fileUrl, $ossFilePath);
                $stmt->execute();
                $stmt->close();
